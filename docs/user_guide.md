...
**2단계: 스캐폴딩 계획서 승인**
- 에이전트가 `harness-100` 및## 7. 실전 운영 팁 및 PC 간 동기화
...
* **해결**: ICIP는 `00_purpose_anchor.md`를 불변으로 유지하는 것을 감시합니다. 만약 훼손되었을 경우, `checkpoint.json` 또는 `Decision Log`에 남아있는 백업본 정보를 바탕으로 오케스트레이터가 목적 앵커 파일을 다시 원래대로 자동 복원합니다.스트 UI 출력**:
  ```powershell
