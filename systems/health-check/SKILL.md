...
### Pass 4: Cleanup & Archive (불필요 중간 파일 정리)
- 코드베이스를 스캔하여 작업이 끝난 중간 찌꺼기 파일들(예: 배포가 끝난 `_workspace/scaffolded_harness/` 하위 소스들, `scratch/` 내의 불필요한 테스트 파일 등)을 식별합니다.
- 식별된 파일들을 아카이브(가상 목록화)하고, 사용자에게 이를 일괄 삭제할지 그대로 유지할지 판단을 묻기 위해 보고서에 추가합니다.

### Pass 5: 보고서 작성 및 사용자 승인
모순 또는 정리 대상 파일이 발견되면 다음 형식으로 보고서를 작성한다 (`implementation_plan.md` 활용):
...
## 🗑️ Cleanup & Archive 제안
- 발견된 임시/찌꺼기 파일 목록:
  1. `_workspace/scaffolded_harness/...`
- **사용자 확인**: "위 불필요한 파일들을 삭제(Delete)할까요, 아니면 유지(Keep)할까요?"
```
- 사용자의 패치 승인(및 삭제 여부)을 대기한다. 모순과 정리 대상이 0건이면 정상 종료한다.

### Pass 6: Zero-Drift Policy (패치 후 즉각 재검증)
- 사용자의 승인 하에 패치 및 삭제를 수행한 직후, **작업을 바로 종료하지 않는다**.
